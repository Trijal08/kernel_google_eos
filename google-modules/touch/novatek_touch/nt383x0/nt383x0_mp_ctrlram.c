...
/*add for short/open/noise test functions */

static void nvt_print_data_log_in_one_line_str(int32_t *data, int32_t data_num, char *ret_tmp_log)
{
	char *tmp_log = NULL;
	int32_t i = 0;
	...
	//copy to show string
	memcpy(ret_tmp_log,tmp_log,data_num * 7);
...
static int32_t nvt_read_baseline(int32_t *xdata)
{
	...
	nvt_print_lmt_array(xdata, X_Channel, Y_Channel);
#endif /* #if NVT_SAVE_TEST_DATA_IN_FILE */
...
#endif /* #if TOUCH_KEY_NUM > 0 */
#else /* #if NVT_SAVE_TEST_DATA_IN_FILE */
	nvt_print_lmt_array(RawData_Diff_Max, X_Channel, Y_Channel);
#endif /* #if NVT_SAVE_TEST_DATA_IN_FILE */
...
#else /* #if NVT_SAVE_TEST_DATA_IN_FILE */
	nvt_print_lmt_array(RawData_Diff_Min, X_Channel, Y_Channel);
#endif /* #if NVT_SAVE_TEST_DATA_IN_FILE */
...
int32_t nvt_mp_parse_dt(void)
{
	...
	for_each_child_of_node(np, child) {
		...
		if (of_device_is_compatible(child, mpcriteria)) {
...
{
	if (nvt_mp_parse_dt()) {
		NVT_LOG("mp parse dt failed, use default setting\n");
		//---Print Test Criteria---
		nvt_print_criteria();
	}

#if NVT_TOUCH_MP_PROC_INIT
	...
	if(nvt_mp_buffer_init()) {
		...
		NVT_LOG("cnvt_mp_proc_init Succeeded!\n");
	}
	return 0;
#endif
}
...
static int32_t nvt_mp_sysfs_printf(char *buf, const char *fmt)
{
	int32_t ret = 0;
...
static int32_t print_sysfs_selftest_result(char *buf, int32_t TestResult, uint8_t RecordResult[], int32_t rawdata[], uint8_t x_len, uint8_t y_len)
{
	int32_t ret = 0;

	switch (TestResult) {
		case 0:
			ret += nvt_mp_sysfs_printf(buf+ret, " PASS!\n");
			break;

		case 1:
			ret += nvt_mp_sysfs_printf(buf+ret, " ERROR! Read Data FAIL!\n");
			break;

		case -1:
			ret += nvt_mp_sysfs_printf(buf+ret, " FAIL!\n");
			break;
	}
...
/* modify for short/open/noise test functions */

static int32_t nvt_show_selftest(char *buf, int show_test_item)
{
	int32_t ret = 0;
	char str[32];
	int i;
	...
	ret_buf = (char *)kzalloc(X_Channel * 7 + 1, GFP_KERNEL);

	NVT_LOG("++\n");

	if (show_test_item == NVT_SELF_TEST)
	{
		sprintf(str, ">> FW Version: %d\n", fw_ver);
		ret += nvt_mp_sysfs_printf(buf+ret, str);
	}

	// --Short Test---
	if (show_test_item == NVT_SELF_TEST || show_test_item == NVT_SHORT_TEST)
	{
		ret += nvt_mp_sysfs_printf(buf+ret, ">> Short Test");
		ret += print_sysfs_selftest_result(buf+ret, TestResult_Short,
			RecordResult_Short, RawData_Short, X_Channel, Y_Channel);

		NVT_LOG(">>> start nvt_show_short_test\n");

		for(i=0; i < Y_Channel; i++)
		{
			nvt_print_data_log_in_one_line(RawData_Short+(i*Y_Channel), X_Channel);
			nvt_print_data_log_in_one_line_str(RawData_Short+(i*Y_Channel), X_Channel,ret_buf);
			ret += nvt_mp_sysfs_printf(buf+ret,ret_buf);
			ret += nvt_mp_sysfs_printf(buf+ret,"\r\n");
		}

		NVT_LOG(">>> end nvt_show_short_test\n");
	}
	// --Short Test---

	// --Open Test---
	if (show_test_item == NVT_SELF_TEST || show_test_item == NVT_OPEN_TEST)
	{
		ret += nvt_mp_sysfs_printf(buf+ret, ">> Open Test");
		ret += print_sysfs_selftest_result(buf+ret, TestResult_Open,
				RecordResult_Open, RawData_Open, X_Channel, Y_Channel);

		NVT_LOG(">>> start nvt_show_open_test\n");

		for(i=0; i < Y_Channel; i++)
		{
			nvt_print_data_log_in_one_line(RawData_Open+(i*Y_Channel), X_Channel);
			nvt_print_data_log_in_one_line_str(RawData_Open+(i*Y_Channel), X_Channel,ret_buf);
			ret += nvt_mp_sysfs_printf(buf+ret,ret_buf);
			ret += nvt_mp_sysfs_printf(buf+ret,"\r\n");
		}

		NVT_LOG(">>> end nvt_show_open_test\n");
	}
	// --Open Test---

	// --FW Rawdata Test---
	if (show_test_item == NVT_SELF_TEST || show_test_item == NVT_FW_RAWDATA_TEST)
	{
		ret += nvt_mp_sysfs_printf(buf+ret, ">> FW Rawdata Test");
		ret += print_sysfs_selftest_result(buf+ret, TestResult_FW_Rawdata,
				RecordResult_FW_Rawdata, RawData_FW_Rawdata, X_Channel, Y_Channel);

		NVT_LOG(">>> start nvt_fw_rawdata_test\n");

		for(i=0; i < Y_Channel; i++)
		{
			nvt_print_data_log_in_one_line(RawData_FW_Rawdata+(i*Y_Channel), X_Channel);
			nvt_print_data_log_in_one_line_str(RawData_FW_Rawdata+(i*Y_Channel), X_Channel,ret_buf);
			ret += nvt_mp_sysfs_printf(buf+ret,ret_buf);
			ret += nvt_mp_sysfs_printf(buf+ret,"\r\n");
		}

		NVT_LOG(">>> end nvt_fw_rawdata_test\n");

	}
	// --FW Rawdata Test---

	// --FW CC Test---
	if (show_test_item == NVT_SELF_TEST || show_test_item == NVT_FW_CC_TEST)
	{
		ret += nvt_mp_sysfs_printf(buf+ret, ">> FW CC Test");
		ret += print_sysfs_selftest_result(buf+ret, TestResult_FW_CC,
				RecordResult_FW_CC, RawData_FW_CC, X_Channel, Y_Channel);

			NVT_LOG(">>> start nvt_fw_cc_test\n");

			for(i=0; i < Y_Channel; i++)
			{
				nvt_print_data_log_in_one_line(RawData_FW_CC+(i*Y_Channel), X_Channel);
				nvt_print_data_log_in_one_line_str(RawData_FW_CC+(i*Y_Channel), X_Channel,ret_buf);
				ret += nvt_mp_sysfs_printf(buf+ret,ret_buf);
				ret += nvt_mp_sysfs_printf(buf+ret,"\r\n");
			}

			NVT_LOG(">>> end nvt_fw_cc_test\n");
	}
	// --FW CC Test---

	// --Noise Test---
	if (show_test_item == NVT_SELF_TEST || show_test_item == NVT_NOISE_TEST)
	{
		ret += nvt_mp_sysfs_printf(buf+ret, ">> Noise Test");
		if ((TestResult_Noise == 0) || (TestResult_Noise == 1)) {
			ret += print_sysfs_selftest_result(buf+ret, TestResult_FW_DiffMax,
				RecordResult_FW_DiffMax, RawData_Diff_Max, X_Channel, Y_Channel);

			NVT_LOG(">>> start nvt_noise_test\n");
			for(i=0; i < Y_Channel; i++)
			{
				nvt_print_data_log_in_one_line(RawData_Diff_Max+(i*Y_Channel), X_Channel);
				nvt_print_data_log_in_one_line_str(RawData_Diff_Max+(i*Y_Channel), X_Channel,ret_buf);
				ret += nvt_mp_sysfs_printf(buf+ret,ret_buf);
				ret += nvt_mp_sysfs_printf(buf+ret,"\r\n");
			}
                        ...
                        for(i=0; i < Y_Channel; i++)
                        {
                                nvt_print_data_log_in_one_line(RawData_Diff_Min+(i*Y_Channel), X_Channel);
                                nvt_print_data_log_in_one_line_str(RawData_Diff_Min+(i*Y_Channel), X_Channel,ret_buf);
                                ret += nvt_mp_sysfs_printf(buf+ret,ret_buf);
                                ret += nvt_mp_sysfs_printf(buf+ret,"\r\n");
                        }

			NVT_LOG(">>> end nvt_fw_noise_test\n");

		} else { // TestResult_Noise is -1
			ret += nvt_mp_sysfs_printf(buf+ret, " FAIL!\n");

			if (TestResult_FW_DiffMax == -1) {
				ret += nvt_mp_sysfs_printf(buf+ret, ">> FW Diff Max");
				ret += print_sysfs_selftest_result(buf+ret, TestResult_FW_DiffMax,
						RecordResult_FW_DiffMax, RawData_Diff_Max, X_Channel, Y_Channel);

				NVT_LOG(">>> start nvt_noise_test fw diff max\n");
				for(i=0; i < Y_Channel; i++)
				{
					nvt_print_data_log_in_one_line(RawData_Diff_Max+(i*Y_Channel), X_Channel);
					nvt_print_data_log_in_one_line_str(RawData_Diff_Max+(i*Y_Channel), X_Channel,ret_buf);
				...
				NVT_LOG(">>> end nvt_fw_noise_test fw diff max\n");
			}
			if (TestResult_FW_DiffMin == -1) {
				ret += nvt_mp_sysfs_printf(buf+ret, ">> FW Diff Min");
				ret += print_sysfs_selftest_result(buf+ret, TestResult_FW_DiffMin,
						RecordResult_FW_DiffMin, RawData_Diff_Min, X_Channel, Y_Channel);

				NVT_LOG(">>> start nvt_noise_test fw diff min\n");
				for(i=0; i < Y_Channel; i++)
				{
					nvt_print_data_log_in_one_line(RawData_Diff_Min+(i*Y_Channel), X_Channel);
					nvt_print_data_log_in_one_line_str(RawData_Diff_Min+(i*Y_Channel), X_Channel,ret_buf);
				...
				NVT_LOG(">>> end nvt_fw_noise_test fw diff min\n");
			}
		}
	}
	// --Noise Test---
...
/* --Short Test--- */
static ssize_t nvt_shorttest_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	ret = nvt_show_selftest(buf,NVT_SHORT_TEST);

	return ret;
}

/* ---Open Test--- */
static ssize_t nvt_opentest_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int32_t ret = 0;
	...
	ret = nvt_show_selftest(buf,NVT_OPEN_TEST);

	return ret;
}

static ssize_t nvt_noisetest_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int32_t ret = 0;
	...
	ret = nvt_show_selftest(buf,NVT_NOISE_TEST);
		...
		struct device_attribute *attr, char *buf)
{
	...
	nvt_mp_test_result_printed = 0;
	/* modify for short open noise test functions */
	//ret = nvt_show_selftest(buf);
	ret = nvt_show_selftest(buf,NVT_SELF_TEST);

	return ret;
}

static DEVICE_ATTR(nvt_selftest, S_IRUGO, nvt_selftest_show, NULL);
/* add for short open noise test functions */
static DEVICE_ATTR(nvt_shorttest, S_IRUGO, nvt_shorttest_show, NULL);
static DEVICE_ATTR(nvt_opentest, S_IRUGO, nvt_opentest_show, NULL);
static DEVICE_ATTR(nvt_noisetest, S_IRUGO, nvt_noisetest_show, NULL);

static struct attribute *nvt_ts_mp_attributes[] = {
	&dev_attr_nvt_selftest.attr,
	/* add for short open noise test functions */
	&dev_attr_nvt_shorttest.attr,
	&dev_attr_nvt_opentest.attr,
	&dev_attr_nvt_noisetest.attr,
	NULL
};

static struct attribute_group nvt_ts_mp_attribute_group = {
	.attrs = nvt_ts_mp_attributes,
};

int32_t nvt_touch_mp_sysfs_init(void)
{
	int32_t ret = 0;

	ret = sysfs_create_group(&ts->input_dev->dev.kobj, &nvt_ts_mp_attribute_group);
	if (ret != 0) {
...
void nvt_touch_mp_sysfs_deinit(void)
{
	sysfs_remove_group(&ts->input_dev->dev.kobj, &nvt_ts_mp_attribute_group);
}
#endif
