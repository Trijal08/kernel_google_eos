...
int32_t Check_FW_Ver(void)
{
	...
	ret = nvt_set_page(I2C_BLDR_Address, ts->mmap->EVENT_BUF_BEGIN_ADDR | EVENT_MAP_FWINFO);
	if (ret < 0) {
...
void Boot_Update_Firmware(struct work_struct *work)
{
	...
	if (ts->fw_name) {
		snprintf(firmware_name, sizeof(firmware_name), ts->fw_name);
	} else {
	...
	NVT_LOG("Boot_Update_Firmware release\n");
}
