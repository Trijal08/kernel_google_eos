	...
	u8 *mac_addr, const char *name)
{
	...
	new_ndev = wl_cfg80211_post_ifcreate(primary_ndev,
		event, addr, name,
		(wl_iftype == WL_IF_TYPE_NAN ? true : false));
		...
		struct net_device *dev, bool suspend)
{
	...
	int bcn_to_dly = 0;
	int bcn_li_dtim = 0;
#ifdef DTIM_MULTIPLIER_FIXED
	bcn_li_dtim = (cfg->suspend_bcn_li_dtim) ? (cfg->suspend_bcn_li_dtim) : 0;
#elif defined(CUSTOM_DTIM_MULTIPLIER)
	bcn_li_dtim = CUSTOM_DTIM_MULTIPLIER;
#endif /* DTIM_MULTIPLIER_FIXED */
...
#if !defined(DISABLE_CFG_GET_CHANNEL) && (LINUX_VERSION_CODE >= KERNEL_VERSION (3, 6, 0))
static int
