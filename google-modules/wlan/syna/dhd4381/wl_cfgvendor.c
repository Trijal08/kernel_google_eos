	...
	wifi_channel_stat *chan_stats, int num_channels, char **output, uint *total_len)
{
	...
	radio_stats_size = WL_RADIOSTAT_SLICE_INDEX_MAX*(offsetof(wifi_radio_stat, channels)) +
			chan_stats_size;
#else
	radio_stats_size = (offsetof(wifi_radio_stat, channels)) +
			chan_stats_size;
...
wl_cfgvendor_simple_hang_event(struct net_device *dev, u16 reason)
{
	...
	WL_ERR(("hang reason: %s, hang pending: %d\n", dhd->memdump_str, dhd->hang_was_pending));
	nla_put(msg, DEBUG_ATTRIBUTE_HANG_REASON, DHD_MEMDUMP_LONGSTR_LEN, dhd->memdump_str);
	if (reason == WLAN_NOTIFY_SIGNATURE) {
		nla_put_u32(msg, DEBUG_ATTRIBUTE_HANG_PENDING, dhd->hang_was_pending);
	}
#endif
