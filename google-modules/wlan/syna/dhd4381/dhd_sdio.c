...
#if defined(LINUX) && (defined(SUPPORT_P2P_GO_PS) || defined(SUSPEND_BUS_SLEEP))
	wait_queue_head_t bus_sleep;
#endif /* LINUX && (SUPPORT_P2P_GO_PS || SUSPEND_BUS_SLEEP) */
...
#ifdef SDIO_ISO_EXT
		devctl = bcmsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, &err);
		if (devctl & SBSDIO_DEVCTL_PADS_ISO) {
			DHD_INFO(("dhdsdio_htclk: request %s rd devctl 0x%x\n",
				(on ? "ON" : "OFF"),
				devctl));
			devctl &= ~SBSDIO_DEVCTL_PADS_ISO;
			bcmsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, devctl, NULL);   /* enable d0-d3 pin */
			devctl = bcmsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, &err);
			DHD_INFO(("dhdsdio_htclk: request %s wr devctl 0x%x\n",
				(on ? "ON" : "OFF"),
				devctl));
		}
#endif /* SDIO_ISO_EXT */
...
dhdsdio_bussleep(dhd_bus_t *bus, bool sleep)
{
	...
	if (sleep) {
		...
		if (!SLPAUTO_ENAB(bus)) {
			 ...
			 */
#ifdef SDIO_ISO_EXT
			if (bus->dhd->early_suspended  == 1) {
				devctl = bcmsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, &err);
				DHD_INFO(("dhdsdio_bussleep: request %s rd devctl 0x%x\n",
					(sleep ? "SLEEP" : "WAKE"),
					devctl));
				devctl |= SBSDIO_DEVCTL_PADS_ISO;
				bcmsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, devctl, NULL);   /* disable d0-d3 pin */
				devctl = bcmsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, &err);
				DHD_INFO(("dhdsdio_bussleep: request %s wr devctl 0x%x\n",
					(sleep ? "SLEEP" : "WAKE"),
					devctl));
			}
#endif /* SDIO_ISO_EXT */
...
dhd_bus_rxctl(struct dhd_bus *bus, uchar *msg, uint msglen)
{
	...
	if (timeleft == 0) {
		...
		if (!bus->dhd->dongle_trap_occured) {
			bus->dhd->alert_reason = ALERT_IOCTL_TIMEOUT;
                     ...
                     uint16 devid)
{
	...
	if (CHIPID(bus->sih->chip) == BCM4381_CHIP_GRPID) {
		int val = 0;
		val = bcmsdh_reg_read(bus->sdh, 0x1801261c,4);
		DHD_ERROR(("0x1801261c=0x%4x\n", bcmsdh_reg_read(bus->sdh, 0x1801261c,4)));
		val |= (1<<27);
		val |= (1<<28);
		bcmsdh_reg_write(bus->sdh, 0x1801261c, 4, val);
		OSL_DELAY(120);
		DHD_ERROR(("0x1801261c=0x%4x\n", bcmsdh_reg_read(bus->sdh, 0x1801261c,4)));

		val = bcmsdh_reg_read(bus->sdh, 0x18012618,4);
		DHD_ERROR(("0x18012618=0x%4x\n", bcmsdh_reg_read(bus->sdh, 0x18012618,4)));
		val |= (1<<27);
		val |= (1<<28);
		bcmsdh_reg_write(bus->sdh, 0x18012618, 4, val);
		OSL_DELAY(120);
		DHD_ERROR(("0x18012618=0x%4x\n", bcmsdh_reg_read(bus->sdh, 0x18012618,4)));

		DHD_ERROR(("0x18012618=0x%4x, 0x1801260c=0x%4x, 0x180331e0=0x%4x, 0x180001e8=0x%x\n",
			bcmsdh_reg_read(bus->sdh, 0x18012618,4), bcmsdh_reg_read(bus->sdh, 0x1801260c,4),
			bcmsdh_reg_read(bus->sdh, 0x180331e0,4), bcmsdh_reg_read(bus->sdh, 0x180001e8,4)));

		bcmsdh_reg_write(bus->sdh, 0x18010040, 2, 0x7);
		DHD_ERROR(("0x180127ec=0x%x, 0x18010040=0x%4x, 0x18010204=0x%4x\n",
			bcmsdh_reg_read(bus->sdh, 0x180127ec,4), bcmsdh_reg_read(bus->sdh, 0x18010040,4),
			bcmsdh_reg_read(bus->sdh, 0x18010204,4)));

		DHD_ERROR(("0x18131800=0x%x, 0x18130800=0x%x, 0x18100800=0x%x, 0x18000000=0x%4x\n",
			bcmsdh_reg_read(bus->sdh, 0x18131800,4), bcmsdh_reg_read(bus->sdh, 0x18130800,4),
			bcmsdh_reg_read(bus->sdh, 0x18100800,4), bcmsdh_reg_read(bus->sdh, 0x18000000,4)));
	}
#endif /* DHD_DEBUG */
...
{
	DHD_ERROR(("%s: Enter bus->dhd %p bus->dhd->dongle_reset %d, dongle_isolation %d\n", __FUNCTION__,
		bus->dhd, bus->dhd->dongle_reset, dongle_isolation));
...
{
	int ret = 0;
#if defined(SUPPORT_P2P_GO_PS) || defined(SUSPEND_BUS_SLEEP)
	int wait_time = 0;
#endif /* SUPPORT_P2P_GO_PS || SUSPEND_BUS_SLEEP */
...
dhd_bus_devreset(dhd_pub_t *dhdp, uint8 flag)
{
	...
	if (flag == TRUE) {
		if (!bus->dhd->dongle_reset) {
			...
			bcmsdh_reg_write(bus->sdh, 0x18010e94, 4, 0x4);
			dhdsdio_advertise_bus_cleanup(bus->dhd);
			dhd_os_sdlock(dhdp);
			...
			if (!dhdp->skip_pmu_pll_reset) {
				/* Remove ARM PLL clock request */
				DHD_ERROR(("%s: Remove ARM PLL clock request \n", __FUNCTION__));
				bcmsdh_reg_write(bus->sdh, 0x180301e0, 4, 0);
				OSL_DELAY(50);
				val = bcmsdh_reg_read(bus->sdh, 0x1801261c,4);
				...
				DHD_ERROR(("%s: memdump happaned, skip Remove ARM PLL clock request \n", __FUNCTION__));

			dhdp->skip_pmu_pll_reset = FALSE;

			/* SDIO pad isolation */
			devctl = bcmsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, &err);
			devctl |= SBSDIO_DEVCTL_PADS_ISO;
			bcmsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, devctl, NULL);   /* disable d0-d3 pin */
