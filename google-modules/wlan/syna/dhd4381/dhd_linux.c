	...
	if (!dhd_download_fw_on_driverload) {
		...
		if (dhd->dhd_state & DHD_ATTACH_LOGTRACE_INIT) {
			if (dhd->event_data.fmts) {
				KVMFREE(dhd->pub.osh, dhd->event_data.fmts,
					dhd->event_data.fmts_size);
			}
			if (dhd->event_data.raw_fmts) {
				KVMFREE(dhd->pub.osh, dhd->event_data.raw_fmts,
		...
		uint32 lr, char *lr_fn)
{
	...
	if (err < 0) {
		DHD_INFO(("dhd_os_get_img(Request Firmware API) is not supported: %d\n",
			err));
		goto fail;
	}
	size = fw->size;
...
dhd_init_logstrs_array(osl_t *osh, dhd_event_log_t *temp)
{
	...
	if (error < 0) {
		DHD_INFO(("dhd_os_get_img(Request Firmware API) file is not supported : %d\n",
			error));
		goto fail;
	}
...
fail:
	...
	if (temp->fmts != NULL) {
		KVMFREE(osh, temp->fmts, temp->num_fmts * sizeof(char *));
	}
...
dhd_init_static_strs_array(osl_t *osh, dhd_event_log_t *temp, char *str_file, char *map_file)
{
	...
	if (error != BCME_OK) {
		DHD_INFO(("No readmap executed. Skip event log parsing.\n"));
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	dhd->pub.dbg->pkts_lock = osl_spin_lock_init(dhd->pub.osh);
...
#if defined(READ_CONFIG_FROM_FILE) && defined(CONFIG_BCMDHD_CONFIG_PATH)
...
void dhd_detach(dhd_pub_t *dhdp)
{
	...
	if (dhdp->dbg) {
		...
		osl_spin_lock_deinit(dhd->pub.osh, dhd->pub.dbg->pkts_lock);
#endif /* DBG_PKT_MON */
	}
#endif /* DEBUGABILITY */
	if (dhdp->dbg) {
...
dhd_os_get_img_fwreq(const struct firmware **fw, char *file_path)
{
	...
	if (ret < 0) {
		DHD_INFO(("%s: request_firmware ret: %d\n", __FUNCTION__, ret));
...
void
wl_apf_buffer_print(u8 *pc, u32 plen)
{
	u32 i;
	char buf[4096] = {0};
	int offset = 0;

	offset += snprintf(buf + offset, sizeof(buf) - offset, "APF program: ");

	for (i = 1; pc && (i <= plen); pc++, i++) {
		offset += snprintf(buf + offset, sizeof(buf) - offset, "%02x", *pc);  // Append each byte value to the buffer
	}
	printf("%s\n", buf);
}
	...
	uint32 program_len)
{
	...
	wl_apf_buffer_print(program, program_len);
...
void dhd_schedule_memdump(dhd_pub_t *dhdp, uint8 *buf, uint32 size)
{
	...
	DHD_ERROR(("%s: skip_pmu_pll_reset set TRUE\n", __FUNCTION__));
	dhdp->skip_pmu_pll_reset = TRUE;
...
#if defined(WL_CFG80211)
		/*
		 * Before SoC RAM dump is written, Sending HANG Event to notify WiFi HAL
		 * the crash reason. WiFi HAL can use DEBUG_ATTRIBUTE_HANG_PENDING to
		 * check the status of coredump process.
		 */
		dev = dhd_linux_get_primary_netdev(dhdp);
		if (dev) {
			wl_cfgvendor_simple_hang_event(dev, WLAN_NOTIFY_SIGNATURE);
		}
#endif
...
int dhd_read_from_file(dhd_pub_t *dhd)
{
	...
	ret = dhd_os_get_img_fwreq(&fw, filepath);
	if (ret < 0) {
		DHD_INFO(("dhd_os_get_img_fwreq(%s) ret : %d\n",
			filepath, ret));
		goto exit;
	}
