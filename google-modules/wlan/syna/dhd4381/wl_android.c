...
#endif /* WBRC */
			if (retry < POWERUP_MAX_RETRY) {
				unsigned long delay_extend =
					(POWERUP_MAX_RETRY - retry + 1) * WIFI_TURNON_DELAY;
				/* Toggle WL_REG_ON and extend delay for test */
				dhd_net_wifi_platform_set_power(dev, TRUE, delay_extend);
				dhd_net_wifi_platform_set_power(dev, FALSE, delay_extend);
				dhd_net_wifi_platform_set_power(dev, TRUE, delay_extend);
			} else {
