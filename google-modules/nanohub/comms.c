...
static int nanohub_comms_tx_rx(struct nanohub_data *data, uint32_t cmd,
			       struct nanohub_packet_pad *pad, int packet_size,
			       uint32_t seq, uint8_t *rx_id, uint8_t *rx,
			       size_t rx_len)
{
	int ret;
	int timeout = cmd == CMD_COMMS_WRITE ? data->comms.timeout_write :
					       data->comms.timeout_read;
				      ...
				      int retrans_cnt, int retrans_delay)
{
	...
	do {
		...
		ret =
		    nanohub_comms_tx_rx(data, cmd, pad, packet_size, seq, rx_id,
					rx, rx_len);
				...
				int retrans_delay)
{
	...
	do {
		...
		data->comms.open(data);
		ret =
		    nanohub_comms_tx_rx(data, cmd, pad, packet_size, seq, rx_id, rx,
					rx_len);
