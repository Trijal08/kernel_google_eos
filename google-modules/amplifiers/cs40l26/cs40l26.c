	...
	if (cs40l26->vibe_state == CS40L26_VIBE_STATE_HAPTIC) {
		...
		if (!wait_for_completion_timeout(&cs40l26->erase_cont,
			...
			dev_err(cs40l26->dev, "Failed to erase effect (%d)\n",
					effect_id);
			if (!cs40l26->reset_pending) {
				cs40l26->reset_pending = true;
		...
		u32 tuning)
{
	...
	if (!cs40l26->dvl_tuning_disable && cl_dsp_algo_is_present(cs40l26->dsp,
			CS40L26_DVL_ALGO_ID))
		strscpy(coeff_files[file_count++], CS40L26_DVL_FILE_NAME,
				CS40L26_FILE_NAME_MAX_LEN);

	if (cs40l26->fw_id == CS40L26_FW_ID) {
		if (!cs40l26->a2h_tuning_disable && cl_dsp_algo_is_present(cs40l26->dsp,
			CS40L26_A2H_ALGO_ID))
...
static int cs40l26_parse_properties(struct cs40l26_private *cs40l26)
{
	...
	cs40l26->a2h_tuning_disable = device_property_present(dev, "cirrus,disable-a2h");
	cs40l26->dvl_tuning_disable = device_property_present(dev, "cirrus,disable-dvl");
...
bool cs40l26_loaded = false;
EXPORT_SYMBOL(cs40l26_loaded);
...
static void cs40l26_reset_worker(struct work_struct *work)
{
	...
	cs40l26->reset_pending = false;
	cs40l26_pm_exit(cs40l26->dev);
	dev_warn(dev, "Reset end");
	goto cleanup;

err:
	dev_err(dev, "Reset end: Fatal error at reset");
cleanup:
	cs40l26_pm_exit(cs40l26->dev);
	cs40l26->reset_pending = false;
}
...
int cs40l26_remove(struct cs40l26_private *cs40l26)
{
	...
	if (cs40l26->vibe_workqueue) {
		...
		cancel_work_sync(&cs40l26->upload_work);
		cancel_work_sync(&cs40l26->erase_work);
		cancel_work_sync(&cs40l26->reset_work);
