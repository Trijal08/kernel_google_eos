...
extern bool cs40l26_loaded;
extern int snd_cs40l26_loaded;
...
static int cs40l26_codec_driver_probe(struct platform_device *pdev)
{
	...
	if (!cs40l26_loaded) {
		/* cs40l26 did not load successfully */
		dev_err(&pdev->dev, "Skipping cs40l26-codec probe\n");
		error = -EIO;
		goto failed;
	}
	...
	if (!codec) {
		dev_err(&pdev->dev, "devm_kzalloc failure\n");
		error = -ENOMEM;
		goto failed;
	}
