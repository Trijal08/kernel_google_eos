...
static int besbev_get_temperature(struct snd_soc_component *component,
				   int *temp);
#ifdef CONFIG_GOOGLE_SW5100_AUDIO
static int besbev_get_temperature_cam_therm(struct snd_soc_component *component,
				   int *temp);
#endif
enum {
	SPKR_STATUS = 0,
...
#ifdef CONFIG_GOOGLE_SW5100_AUDIO
		besbev->micbias_en[micb_index] =  true;
#endif
		break;
	case MICB_DISABLE:
...
#ifdef CONFIG_GOOGLE_SW5100_AUDIO
		besbev->micbias_en[micb_index] =  false;
#endif
		break;
	};
...
#ifdef CONFIG_GOOGLE_SW5100_AUDIO
static int besbev_codec_micbias_get(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
	struct besbev_priv *besbev = snd_soc_component_get_drvdata(component);
	int micb_num, micb_index;

	if (strnstr(kcontrol->id.name, "MIC BIAS1", sizeof("MIC BIAS1")))
		micb_num = MIC_BIAS_1;
	else if (strnstr(kcontrol->id.name, "MIC BIAS2", sizeof("MIC BIAS2")))
		micb_num = MIC_BIAS_2;
	else
		return -EINVAL;

	micb_index = micb_num - 1;
	ucontrol->value.integer.value[0] = besbev->micbias_en[micb_index];

	dev_dbg(component->dev, "%s: micbias_en[%d] = 0x%x\n", __func__,
		micb_index, besbev->micbias_en[micb_index]);

	return 0;
}

static int besbev_codec_micbias_put(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
...
#ifdef CONFIG_GOOGLE_SW5100_AUDIO
		if (besbev->cam_therm_en)
			besbev_get_temperature_cam_therm(component, &temp);
		else
...
static const struct snd_kcontrol_new besbev_snd_controls_tx[] = {
	...
	SOC_SINGLE_EXT("MIC BIAS1", SND_SOC_NOPM, 0, 1, 0,
		besbev_codec_micbias_get,
		besbev_codec_micbias_put),
				   ...
				   int *temp)
{
	...
	u8 retry = BESBEV_TEMP_RETRY;
	struct besbev_priv *besbev = snd_soc_component_get_drvdata(component);
	struct iio_channel *ch_cam_therm;

	if (!besbev)
		return -EINVAL;

	ch_cam_therm = iio_channel_get(besbev->dev, "cam_therm");
	if (ch_cam_therm == NULL) {
		pr_err("could not get iio channel name\n");
		ret = -EINVAL;
		goto clean_ch_name;
	}
	do {
		pr_debug("read temperature from camera thermistor\n");
		ret = iio_read_channel_processed(ch_cam_therm, &temp_val);

		pr_debug("cam thermistor temp value is %d\n", temp_val);
		/*
		 * temp_val is in millidegrees C; add 500 and then divide by 1000
		 * to get the result to properly rounded degrees C.
		 */
		temp_val = (temp_val + 500) / 1000;
...
clean_ch_name:
	iio_channel_release(ch_cam_therm);
	besbev->curr_temp = temp_val;
	if (temp)
		*temp = temp_val;

	pr_debug("%s: t0 measured: %d\n", __func__, temp_val);
...
static int besbev_soc_codec_probe(struct snd_soc_component *component)
{
	...
	besbev->dev_up = true;
#ifdef CONFIG_GOOGLE_SW5100_AUDIO
	if (besbev->mic_bias1_always_on)
		besbev_micbias_control(component, MIC_BIAS_1, MICB_ENABLE, true);
#endif
...
#ifdef CONFIG_GOOGLE_SW5100_AUDIO
	/*
	 * On some HW revisions we want to always enable MIC BIAS1.
	 */
	pdata->mic_bias1_always_on = of_property_read_bool(dev->of_node,
					"google,mic-bias1-always-on");
        pdata->cam_therm_en = of_property_read_bool(dev->of_node,
					"qcom,cam-therm-enabled");
#endif
...
#ifdef CONFIG_GOOGLE_SW5100_AUDIO
	besbev->mic_bias1_always_on = pdata->mic_bias1_always_on;
	besbev->cam_therm_en = pdata->cam_therm_en;
#endif
